...
Look at the existing tree in `publicnotes.html` (the `data-slug` values) and the existing notes in `publicnotes/notes-src/*.md` before deciding. Pick whichever of these three is the best fit — don't default to "new node":
...
One line per item: what it is, which outcome you picked and why, proposed parent/target slug, proposed wikilinks. Wait for Owen to approve or redirect before writing anything that touches `publicnotes.html` or existing note files.
...
- Never invent a parent slug — it must already exist as a `data-slug` in `publicnotes.html`, or have just been created by this same run.
